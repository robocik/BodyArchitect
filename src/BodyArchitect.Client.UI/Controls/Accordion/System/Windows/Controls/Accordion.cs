...
using BodyArchitect.Client.UI.Controls.Accordion.System.Windows;

namespace System.Windows.Controls
{
    /// <summary>
    /// Represents a collection of collapsed and expanded AccordionItem controls.
    /// Patch for accordion item heights: http://wpf.codeplex.com/discussions/203719
    /// </summary>
    ...
    public class Accordion : ItemsControl, IUpdateVisualState
    {
        ...
        private static void OnExpandDirectionPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
                ...
                expandDirection != ExpandDirection.Right)
            {
                ...
                string message = string.Format(
                    CultureInfo.InvariantCulture,"Accordion_OnExpandDirectionPropertyChanged_InvalidValue",
                    expandDirection);
        ...
        private static void OnSelectionModePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
                ...
                newValue != AccordionSelectionMode.ZeroOrOne)
            {
                ...
                string message = string.Format(
                    CultureInfo.InvariantCulture,"Accordion_OnSelectionModePropertyChanged_InvalidValue",
                    newValue);
        ...
        private static void OnSelectionSequencePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
                ...
                newValue != SelectionSequence.Simultaneous)
            {
                ...
                string message = string.Format(
                    CultureInfo.InvariantCulture,"Accordion_OnSelectionSequencepropertyChanged_InvalidValue",
                    newValue);
                throw new ArgumentOutOfRangeException("e", message);
            }
        }
        ...
        private void UnselectItem(int index, object item)
        {
            ...
            if (SelectedItems.OfType<object>().Contains(item) && index != newSelectedIndex && !item.Equals(SelectedItem))
            {
                ...
                if (SelectedIndices.Count(i => i != index && i < Items.Count && Items[i].Equals(item)) == 0)
                {
                    ...
                    {
                        throw new InvalidOperationException("Accordion_InvalidManipulationOfSelectionCollections");
                    }
                    SelectedItems.Remove(item);
                }
            }
        ...
        private void OnSelectedItemsCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            switch (e.Action)
            {
                ...
                default:
                    {
                        string message = string.Format(
                                CultureInfo.InvariantCulture,"Accordion_UnsupportedCollectionAction",
                                e.Action);
        ...
        private void LayoutChildren()
        {
            ...
            {
                //bug fixing in Accordion control. Without this when you restore and maximize window then accordion is not resized properly
                root.UpdateLayout();
