...
namespace BodyArchitect.Controls.External
{
	...
	public class TreeListView : Control, ISupportInitialize
	{
		...
		public virtual TreeListColumnCollection Columns
		{
        ...
        public void Sort(int column,bool asc)
        {
            Sort(column, this.Nodes, asc);
        }

	    private int sortColumnIndex = -1;
  
        private void Sort(int column, NodeCollection nodes,bool asc)
        {
            nodes.Sort(column, asc);
            foreach (Node n in nodes)
            {
                Sort(column, n.Nodes, asc);
            }
        }
		...
		public BodyArchitect.Controls.External.HitInfo CalcColumnHit(Point mousepoint)
		{
		...
		void UpdateScrollBars()
		{
			...
			else
			{
                ...
                //m_vScroll.Maximum = totalrows-1; by
                if (totalrows <= 1)
                    m_vScroll.Maximum = 1;
                else 
                    m_vScroll.Maximum = totalrows - 1; 
		...
		protected override void OnMouseClick(MouseEventArgs e)
		{
			if (e.Button == MouseButtons.Left)
			{
                ...
                else // column click sorting
                {
                    HitInfo info = this.Columns.CalcHitInfo(mousePoint, this.HScrollValue());
                    if (info.Column != null)
                    {
                        if (sortColumnIndex != info.Column.Index)
                        {
                            sortAsc = true;
                        }
                        else
                        ...
                        sortColumnIndex = info.Column.Index;
                        this.Sort(info.Column.Index, sortAsc);
                    }
                }
			}
			base.OnMouseClick(e);
		...
		protected override void OnMouseMove(MouseEventArgs e)
		{
			...
			BodyArchitect.Controls.External.HitInfo info = Columns.CalcHitInfo(new Point(e.X, e.Y), HScrollValue());
		...
		protected override CreateParams CreateParams
		{
			get
			{
				...
				p.Style &= ~(int)BodyArchitect.Controls.External.WinUtil.WS_BORDER;
				p.ExStyle &= ~(int)BodyArchitect.Controls.External.WinUtil.WS_EX_CLIENTEDGE;
		...
		protected virtual BodyArchitect.Controls.External.TreeList.TextFormatting GetFormatting(BodyArchitect.Controls.External.Node node, BodyArchitect.Controls.External.TreeListColumn column)
		{
