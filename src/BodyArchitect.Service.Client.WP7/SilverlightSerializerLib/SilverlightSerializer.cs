...
namespace BodyArchitect.Service.Client.WP7
{
    ...
    /// <summary>
    /// Ten atrybut okresla czy dana własność (narazie działa tylko na własnościach!) jest częścią obiektu czy jest tylko referencją.
    /// Przykład. W treningu siłowym mamy ćwiczenia ale te ćwiczenia są tylko referencją. Chodzi o to ze jak robimy kopię treningu siłowego,
    /// to chcemy żeby wszystkie Id zostały wyzerowane oprócz właśnie ćwiczeń. Id ćwiczen muszą pozostać te same. W takim układzie
    /// oznaczamy własność Exercise w treningu siłowym na UseSameReference i wtedy nawet jesli serializer działa w trybie IgnoreIds
    /// to dla własności z tym atrybutem nie będzie czyscił Id
    /// </summary>
    [AttributeUsage(AttributeTargets.Property | AttributeTargets.Field | AttributeTargets.Class)]
    public class UseSameReference : Attribute
    {
    ...
    public static class SilverlightSerializer
    {
        ...
        {
            //BUG: IgnoreIds in Copy doesn't work. apply workaround
            GetWritableAttributes.PropertyAccess.Clear();
            //GetWritableAttributes.Vanilla.Clear();
        }
        ...
        private static void WriteProperties(Type itemType, object item, IStorage storage)
        {
            ...
            {
                var useSameREferences = entry.PropertyInfo.GetCustomAttributes(typeof (UseSameReference), true).Count() > 0 ||
                          entry.PropertyInfo.GetCustomAttributes(typeof (DoNotSerialize), true).Count() > 0;

                var tmpIgnoreIds = IgnoreIds;
                if(useSameREferences)
                {
                    IgnoreIds = false;
                }
