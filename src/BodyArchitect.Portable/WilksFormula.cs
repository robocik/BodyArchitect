...
{
    public static class WilksFormula
    {
        /// <summary>
        ...
        /// <returns></returns>
        public static string PaceToString(this float pace,bool showWithoutHours=false)
        {
            StringBuilder builder = new StringBuilder();
            var s = Math.Floor(pace); 	  // Whole seconds
        ...
        {
            return PaceToString(timeInSec/distanceInKm);
        }
        //min/km
        public static float SpeedToPace(float speedMetersPerSecond,bool toMiles=false)
        {
            var speedKmPerH = speedMetersPerSecond*3.6f;//to km/h
            if (speedKmPerH == 0f)
            {
                return 0;
            }
        ...
        /// <summary>
        /// Ta metoda oblicza punkty wilksa (bez korzystania z tabel). Mogą być minimalne różnice w porównianiu z wartościami z tabel (na setnych miejscach po przecinku)
        /// </summary>
        /// <param name="bodyWeight"></param>
        /// <param name="exerciseWeight"></param>
        /// <returns></returns>
        public static decimal CalculateForMen(decimal bodyWeight, decimal exerciseWeight)
        {
            var a = -216.0475144m;
            var b = 16.2606339m;
            var c = -0.002388645m;
            ...
            b = b * bodyWeight;
            c = c * bodyWeight * bodyWeight;
            d = d * bodyWeight * bodyWeight * bodyWeight;
            e = e * bodyWeight * bodyWeight * bodyWeight * bodyWeight;
            f = f * bodyWeight * bodyWeight * bodyWeight * bodyWeight * bodyWeight;
            var fCoefficient = (a + b + c + d + e + f);
            var result = (exerciseWeight * 500m) / fCoefficient;
            return result;
        }

        /// <summary>
        /// Ta metoda głównie korzysta z tabel do obliczenia punktów wilksa. Dla wartości z poza zakresu mamy dwie metody obliczania:
        /// dla fromCalculation = true - korzystamy z metody CalculateForMen
        /// dla fromCalculation = false - korzystam ze sposobu obliczeń stąd: http://www.powerlifting.pl/referees/wilks/wilksmalawaga.html. Wyniki róznią sie znacznie od obliczanych pierwszym sposobem wiec ciezko wyczuć co jest ok
        /// </summary>
        /// <param name="bodyWeight"></param>
        /// <param name="exerciseWeight"></param>
        /// <param name="fromCalculations"></param>
        /// <returns></returns>
        public static decimal CalculateForMenUsingTables(decimal bodyWeight, decimal exerciseWeight,bool fromCalculations=true)
        {
            decimal result = 0;
            if (bodyWeight >= 40 && bodyWeight <= 205.9m)
            {

                for (var i = 40; i < 206; i++)
                {

                    if (bodyWeight >= i && bodyWeight < i + 1)
                    {

                        var tempInt = i;

                        var tempDec = Math.Round((bodyWeight - i) * 10);

                        result = (decimal)wilksMenArray[(int)((tempInt - 40) * 10 + tempDec)] * exerciseWeight;
            ...
            else
            {
                if(fromCalculations)
                {
                    result = CalculateForMen(bodyWeight, exerciseWeight);
                }
                else
                {
                    //ta częśc jest wynika min stąd: http://www.powerlifting.pl/referees/wilks/wilksmalawaga.html
                    //niestety na żadnej stronie z kalkulatorem nie dostawałem takich wyników także nie wiem czy tak powinno się liczyć dlatego domyslnie parametr
                    //fromCalculation jest na true bo tamten wynik jest 
                    if (bodyWeight < 40)
                    {
                        var wilksFactor = 1.3354m;
                        var wsp = wilksFactor + .043m * (40 - bodyWeight);
                        result = exerciseWeight * wsp;
                    ...
                    else
                    {
                        ...
                        var wilksFactor = 0.5318m;

                        result = exerciseWeight * wilksFactor;
        ...
        public static decimal CalculateForWomen(decimal bodyWeight, decimal exerciseWeight)
        {
            var a = 594.31747775582m;
            var b = -27.23842536447m;
            var c = 0.82112226871m;
            var d = -0.00930733913m;
            var e = 0.00004731582m;
            var f = -0.00000009054m;
        ...
        public static decimal CalculateForWomenUsingTables(decimal bodyWeight,decimal exerciseWeight,bool fromCalculations=true)
        {
            decimal result = 0;
            if (bodyWeight >= 40 && bodyWeight <= 150.9m)
            {
                for (var i = 40; i < 151; i++)
                {
                    ...
                    if (bodyWeight >= i && bodyWeight < i + 1)
                    {
                        ...
                        result= (decimal) wilksWomenArray[(int) ((tempInt - 40)*10 + tempDec)]*exerciseWeight;
                    }
        ...
        #region Wilks tables
        static readonly double[] wilksWomenArray = new double[]{
...
0.7695,0.7694,0.7694,0.7693,0.7693,0.7692,0.7692,0.7691,0.7691,0.7691};
        ...
        static readonly double[] wilksMenArray = new double[]{
...
0.5317,0.5317,0.5317,0.5317,0.5318,0.5318,0.5318,0.5318,0.5318,0.5318};
    ...
    <OPTION VALUE="2.65" SELECTED>Aerobic Dance (Casual)
    <OPTION VALUE="3">Aerobic Dance (Moderate)
    <OPTION VALUE="3.71">Aerobic Dance (Intense)
    <OPTION VALUE="2.7">Basketball (Full court)
    <OPTION VALUE="1.67">Basketball (Half court)
    <OPTION VALUE="2">Calisthenics
    <OPTION VALUE="1.6">Cycling (5.5 mph, 9 kmph)
    <OPTION VALUE="3">Cycling (10 mph, 16 kmph)
    <OPTION VALUE="4.26">Cycling (13 mph, 21 kmph)
    <OPTION VALUE="4.66">Cycling (Racing)
    <OPTION VALUE="2.62">Dancing (Casual)
    <OPTION VALUE="2.84">Dancing (Moderate)
    <OPTION VALUE="3.65">Dancing (Extreme)
    <OPTION VALUE="3.7">Field Sports (Competitive)
    <OPTION VALUE="2.75">Frisbie
    <OPTION VALUE="2.37">Golf (Carry Clubs)
    <OPTION VALUE="2">Golf (with Cart)
    <OPTION VALUE="1.96">Gymnastics
    <OPTION VALUE="1.7">Housework
    <OPTION VALUE="4.6">Jump Rope (Fast)
    <OPTION VALUE="3.7">Jump Rope (Slow)
    <OPTION VALUE="0.6">Lying
    <OPTION VALUE="5.35">Martial Arts
    <OPTION VALUE="4.88">Racquetball
    <OPTION VALUE="3.6">Rollerblading (Casual)
    <OPTION VALUE="4.25">Rollerblading (Fast)
    <OPTION VALUE="4.91">Rowing Machine (Extreme)
    <OPTION VALUE="3.27">Rowing Machine (Moderate)
    <OPTION VALUE="2.92">Running (4 mph, 6.5 kmph)
    <OPTION VALUE="3.62">Running (5 mph, 8 kmph)
    <OPTION VALUE="4.8">Running (6 mph, 9.7 kmph)
    <OPTION VALUE="5.4">Running (7 mph, 11.3 kmph)
    <OPTION VALUE="6.3">Running (8 mph, 13 kmph)
    <OPTION VALUE="7.5">Running (10 mph, 16 kmph)
    <OPTION VALUE="7.9">Running (12 mph, 19.3 kmph)
    <OPTION VALUE="0.76">Sitting (Office work)
    <OPTION VALUE="0.6">Sitting (Quietly)
    <OPTION VALUE="1.91">Softball
    <OPTION VALUE="3.71">Stair Step (6&quot;, 15.2 cm)
    <OPTION VALUE="4.09">Stair Step (8&quot;, 20.3 cm)
    <OPTION VALUE="4.42">Stair Step (10&quot;, 25.4 cm)
    <OPTION VALUE="4.91">Stair Step (12&quot;, 30.5 cm)
    <OPTION VALUE="0.98">Standing (Office work)
    <OPTION VALUE="0.76">Standing (Quietly)
    <OPTION VALUE="3">Swimming (Casual)
    <OPTION VALUE="10">Swimming (Vigorous)
    <OPTION VALUE="2.56">Tennis (Doubles)
    <OPTION VALUE="3">Tennis (Singles)
    <OPTION VALUE="1.39">Volleyball (Casual)
    <OPTION VALUE="3.98">Volleyball (Vigorous)
    <OPTION VALUE="1.4">Walking (2 mph, 3.2 kmph)
    <OPTION VALUE="1.75">Walking (2.5 mph, 4 kmph)
    <OPTION VALUE="2">Walking (3 mph, 4.8 kmph)
    <OPTION VALUE="2.3">Walking (3.5 mph, 5.6 kmph)
    <OPTION VALUE="2.67">Walking (4 mph, 6.5 kmph)
    <OPTION VALUE="3.1">Walking (4.5 mph, 7.2 kmph)
    <OPTION VALUE="3.2">Weight Training (Intense)
    <OPTION VALUE="2.35">Weight Training (Normal)
    <OPTION VALUE="3.4">Yardwork (Heavy)
    <OPTION VALUE="1.6">Yardwork (Regular)
    </SELECT> Activity</TD>          *          *          * <OPTION VALUE="1" SELECTED>Pounds
    <OPTION VALUE="2.2">Kilograms         * function calc (form) {
var W, T
...
form.Calories.value = Math.round(form.Activity.options[form.Activity.selectedIndex].value*W*T)
        ...
        /// </summary>
        public static decimal CalculateCalories(decimal weightKg, TimeSpan time, decimal activityRatio)
        {
            var weightInPounds = weightKg / 0.454M;
            var calories = Math.Round(activityRatio * (decimal)time.TotalHours * weightInPounds);
            return calories;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static decimal CalculateCaloriesUsingMET(bool male,decimal met,TimeSpan time,decimal age,decimal weight,decimal? height=null)
        {
            decimal bmr = 0;
            if (male)
            {
                ...
                bmr = (13.75M*weight) + (5*height.Value) - (6.76M*age) + 66;
            }
            else
            {//female
                if (height == null)
                ...
                bmr = (9.56M*weight) + (1.85m*height.Value) - (4.68m*age) + 655;
            }
            decimal calories = bmr/24*met*(decimal) time.TotalHours;
