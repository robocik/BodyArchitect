...
{
    /// <summary>
    /// http://nabierz-masy.com/przyklady-zastosowania?start=2 (second)
    /// </summary>
    [TestFixture]
    public class TestService_Anabolic5:TestServiceBase
    {
        ...
        private SupplementCycleDefinition createCycle_1()
        {
            ...
            dosageDto.Supplement = metandienon;
            dosageDto.TimeType = Service.V2.Model.TimeType.NotSet;
            week.Dosages.Add(dosageDto);
        ...
        private SupplementCycleDefinition createCycle_2()
        {
            var cycleDefinition = new SupplementCycleDefinitionDTO();
            cycleDefinition.Name = "creatine cycle";
            cycleDefinition.Language = "en";
        ...
        public void Test_1()
        {
            ...
            Assert.AreEqual(76, dbCycle.EntryObjects.Count);
            ...
            foreach (var entry in entries)
            {
                ...
                if (isMonday)
                {
                    ...
                    Assert.AreEqual(500, item.Dosage);
                    Assert.AreEqual(Model.DosageType.MiliGrams, item.DosageType);
                    Assert.AreEqual(Model.TimeType.NotSet, item.Time.TimeType);
                }

                if (week>=1 && week<=6 || week>=9 && week<=12)
                {
                    ...
                    else
                    {
                        ...
                        Assert.AreEqual(30, item.Dosage);
                        Assert.AreEqual(Model.DosageType.MiliGrams, item.DosageType);
                        Assert.AreEqual(metandienon.GlobalId, item.Suplement.GlobalId);
                        Assert.AreEqual(Model.TimeType.NotSet, item.Time.TimeType);
                    }
                ...
                {
                    Assert.AreEqual(isMonday ? 2 : 0, entry.Items.Count);
                }
            }
        }
