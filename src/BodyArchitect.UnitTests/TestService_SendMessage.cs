...
using ContentType = BodyArchitect.Service.Model.ContentType;
using MessagePriority = BodyArchitect.Service.Model.MessagePriority;
using MessageType = BodyArchitect.Service.Model.MessageType;
    ...
    public class TestService_SendMessage:TestServiceBase
    {
        ...
        public void TestSendCustomMessageToTheUser()
        {
            ...
            RunServiceMethod(delegate(InternalBodyArchitectService Service)
            {
                ...
                message.Topic = "test topic";
                message.Content = "test content";
                message.ContentType = ContentType.Rtf;
                Service.SendMessage(data.Token, message);
            });
            var messageDb =Session.QueryOver<Message>().SingleOrDefault();
            Assert.IsNotNull(messageDb);
            Assert.AreEqual(Model.MessageType.Custom,messageDb.MessageType);
            Assert.AreEqual(Model.ContentType.Rtf, messageDb.ContentType);
            Assert.AreEqual("test topic", messageDb.Topic);
            Assert.AreEqual("test content", messageDb.Content);
            Assert.AreEqual(profile.Id, messageDb.Sender.Id);
            Assert.AreEqual(profile1.Id, messageDb.Receiver.Id);
        }
